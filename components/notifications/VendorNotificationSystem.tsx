...
export function VendorNotificationSystem({ onNewOrder }: VendorNotificationSystemProps) {
  const { user } = useAuth();
    ...
    <Card className="fixed top-4 right-4 w-80 z-50 shadow-lg border-orange-200">
      <CardContent className="p-4">
        ...
        <div className="flex gap-2 mt-3">
          <Button size="sm" variant="outline" onClick={toggleSound} className="w-full">
            {state.soundEnabled ? (
              <><Volume2 className="w-3 h-3 mr-1" />Sonidos ON</>
            ) : (
              <><VolumeX className="w-3 h-3 mr-1" />Sonidos OFF</>
            )}
          </Button>
        </div>
      </CardContent>
