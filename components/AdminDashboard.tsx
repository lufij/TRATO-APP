...
import { SystemSettingsSimple } from './SystemSettingsSimple';
...
function UsersManagement() {
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesStatus = statusFilter === 'all' || (user.is_active !== undefined ? (user.is_active ? 'active' : 'inactive') : 'active') === statusFilter;
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <Badge 
                        variant={(user.is_active !== undefined ? (user.is_active ? 'active' : 'inactive') : 'active') === 'active' ? 'default' : 'destructive'}
                        className={`${
                          (user.is_active !== undefined ? (user.is_active ? 'active' : 'inactive') : 'active') === 'active' ? 'bg-green-100 text-green-800' :
                          (user.is_active !== undefined ? (user.is_active ? 'active' : 'inactive') : 'active') === 'inactive' ? 'bg-gray-100 text-gray-800' :
                          'bg-red-100 text-red-800'
                        }`}
                      >
                        {user.is_active !== undefined ? (user.is_active ? 'active' : 'inactive') : 'active'}
                      </Badge>
                    </TableCell>
                    <TableCell>
...
export function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-green-50">
      ...
      <NotificationSystem 
        showBanner={true}
        enableAutoActivation={false}  // Manual para admins
