-- 🚨 SOLUCION DEFINITIVA PARA ERROR DE ORDENES (CORREGIDA)
...
BEGIN
    ...
    IF NEW.total IS NOT NULL AND (OLD.total IS NULL OR NEW.total != OLD.total) THEN
        NEW.total_amount = NEW.total;
    END IF;
    
    -- Si se actualiza total_amount, sincronizar con total
    IF NEW.total_amount IS NOT NULL AND (OLD.total_amount IS NULL OR NEW.total_amount != OLD.total_amount) THEN
