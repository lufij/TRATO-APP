-- SOLUCIÓN PARA DINE-IN: FUNCIONES MÁS FLEXIBLES
-- ==============================================

-- Función seller_mark_completed_pickup MEJORADA (acepta más tipos)
...
BEGIN
    ...
    -- VERIFICACIÓN MÁS FLEXIBLE: acepta pickup, dine-in, dine_in, y cualquier cosa que NO sea delivery
    IF v_delivery_type = 'delivery' THEN
        RETURN QUERY SELECT false, 'Esta función no es para órdenes de delivery. Use la función de repartidor.'::TEXT;
        RETURN;
    END IF;
    ...
    BEGIN
        CASE 
            WHEN v_delivery_type IN ('pickup') THEN
                v_message := 'Orden pickup completada exitosamente';
            WHEN v_delivery_type IN ('dine_in', 'dine-in') THEN
                v_message := 'Orden dine-in servida exitosamente';
            ELSE
                v_message := 'Orden completada exitosamente (tipo: ' || v_delivery_type || ')';
        END CASE;
    END;
...
-- Función seller_mark_completed_pickup_safe ULTRA PERMISIVA
...
BEGIN
    ...
    -- NO HAY RESTRICCIÓN DE TIPO - acepta cualquier delivery_type que no sea delivery
    ...
    RETURN QUERY SELECT true, ('Orden completada (tipo: ' || COALESCE(v_delivery_type, 'desconocido') || ')')::TEXT;
