...
CREATE TABLE IF NOT EXISTS user_addresses (
    ...
    label VARCHAR(50) NOT NULL, -- 'Casa', 'Trabajo', 'Otro'
    ...
    accuracy_meters INTEGER, -- Precisión de la ubicación en metros
    
    -- Referencias de ubicación
    google_place_id VARCHAR(255), -- ID de Google Places para verificación
    what3words VARCHAR(255), -- Código What3Words como referencia alternativa
    
    -- Detalles adicionales para repartidores
    delivery_instructions TEXT, -- "Tocar el timbre dos veces", "Casa azul", etc.
    landmark TEXT, -- "Frente al parque", "Lado de la farmacia", etc.
    access_notes TEXT, -- "Portón negro", "Segundo piso", etc.
    ...
    building_type VARCHAR(20), -- 'house', 'apartment', 'office', 'building'
    floor_number VARCHAR(10), -- Número de piso
    apartment_number VARCHAR(20), -- Número de apartamento/oficina
    
    -- Tiempos de disponibilidad (para entregas)
    available_from TIME, -- Hora desde cuando recibe entregas
    available_to TIME, -- Hora hasta cuando recibe entregas
    available_days JSONB, -- ['monday', 'tuesday', ...] días disponibles
    ...
    delivery_success_rate DECIMAL(5,2) DEFAULT 100.00, -- Porcentaje de entregas exitosas
...
-- 2. AGREGAR COLUMNAS A TABLA USERS PARA UBICACIÓN ACTUAL
-- =================================================================

DO $
BEGIN
    -- Agregar columnas de ubicación actual al perfil del usuario
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'current_latitude') THEN
        ALTER TABLE users ADD COLUMN current_latitude DECIMAL(10, 8);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'current_longitude') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'location_permission_granted') THEN
        ALTER TABLE users ADD COLUMN location_permission_granted BOOLEAN DEFAULT FALSE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'primary_address_id') THEN
        ALTER TABLE users ADD COLUMN primary_address_id UUID;
    END IF;
    
    RAISE NOTICE 'Columnas de ubicación agregadas a tabla users';
END $;

-- 2.5. AGREGAR COLUMNAS A TABLA ORDERS PARA UBICACIÓN DETALLADA
-- =================================================================

DO $
BEGIN
    -- Agregar columna para almacenar información detallada de ubicación
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'delivery_location') THEN
        ...
        RAISE NOTICE 'Columna delivery_location agregada a tabla orders';
    ELSE
        RAISE NOTICE 'Columna delivery_location ya existe en tabla orders';
    END IF;
    
    -- Agregar columnas de ubicación de recogida si no existen
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'pickup_latitude') THEN
        ALTER TABLE orders ADD COLUMN pickup_latitude DECIMAL(10, 8);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'pickup_longitude') THEN
        ALTER TABLE orders ADD COLUMN pickup_longitude DECIMAL(11, 8);
    END IF;
    
    RAISE NOTICE 'Columnas de ubicación agregadas a tabla orders';
END $;

-- 3. CREAR TABLA LOCATION_VERIFICATION_LOG (Historial de verificaciones)
-- =================================================================
...
CREATE TABLE IF NOT EXISTS location_verification_log (
    ...
    confidence_score DECIMAL(5,2), -- 0-100, qué tan confiable es la verificación
    ...
    order_id UUID, -- Si fue verificado por una entrega exitosa
    driver_id UUID, -- Repartidor que confirmó la dirección
    delivery_confirmation_rating INTEGER, -- 1-5, qué tan fácil fue encontrar la dirección
...
-- Índices para location_verification_log
...
-- 5. CREAR CONSTRAINST Y VALIDACIONES
-- =================================================================

-- Solo puede haber una dirección primaria por usuario
DO $$
BEGIN
    -- Agregar constraint único para dirección primaria
    BEGIN
        ALTER TABLE user_addresses ADD CONSTRAINT user_addresses_one_primary_per_user 
        EXCLUDE (user_id WITH =) WHERE (is_primary = TRUE);
        RAISE NOTICE 'Constraint de dirección primaria única creada';
    EXCEPTION WHEN duplicate_object THEN
        RAISE NOTICE 'Constraint de dirección primaria única ya existe';
    END;
    
    -- Agregar foreign key para primary_address_id en users
    BEGIN
        ALTER TABLE users ADD CONSTRAINT fk_users_primary_address 
        FOREIGN KEY (primary_address_id) REFERENCES user_addresses(id) ON DELETE SET NULL;
        RAISE NOTICE 'Foreign key users -> user_addresses creada';
    EXCEPTION WHEN duplicate_object THEN
        RAISE NOTICE 'Foreign key users -> user_addresses ya existe';
    END;
END $$;

-- 6. FUNCIONES AUXILIARES PARA GESTIÓN DE UBICACIONES
-- =================================================================

-- Función para calcular distancia entre dos puntos GPS (reutilizada)
CREATE OR REPLACE FUNCTION calculate_distance_accurate(lat1 DECIMAL, lng1 DECIMAL, lat2 DECIMAL, lng2 DECIMAL)
RETURNS DECIMAL AS $$
DECLARE
    earth_radius CONSTANT DECIMAL := 6371000; -- Radio de la Tierra en metros
    dlat DECIMAL;
    dlng DECIMAL;
    a DECIMAL;
    c DECIMAL;
BEGIN
    ...
    RETURN earth_radius * c; -- Retorna distancia en metros
END;
$$ LANGUAGE plpgsql;

-- Función para establecer dirección primaria
...
-- Función para buscar direcciones cercanas (para sugerir direcciones similares)
...
-- Función para actualizar estadísticas de uso de dirección
...
CREATE POLICY "Drivers can view delivery addresses" ON user_addresses
    FOR SELECT USING (
        -- Los repartidores pueden ver direcciones de órdenes asignadas a ellos
        EXISTS (
...
-- 9. CONFIGURAR REALTIME PARA ACTUALIZACIONES DE UBICACIÓN
-- =================================================================

DO $$
BEGIN
    -- Agregar tablas a realtime para actualizaciones en tiempo real
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE user_addresses;
        RAISE NOTICE 'Tabla user_addresses agregada a realtime';
    EXCEPTION WHEN duplicate_object THEN
        RAISE NOTICE 'Tabla user_addresses ya está en realtime';
    END;
    
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE location_verification_log;
        RAISE NOTICE 'Tabla location_verification_log agregada a realtime';
    EXCEPTION WHEN duplicate_object THEN
        RAISE NOTICE 'Tabla location_verification_log ya está en realtime';
    END;
END $$;

-- 10. CREAR VISTAS ÚTILES PARA UBICACIONES
-- =================================================================
...
BEGIN
    ...
    -- Verificar columnas en user_addresses
    SELECT COUNT(*) INTO addresses_columns
    FROM information_schema.columns
    WHERE table_schema = 'public' 
    AND table_name = 'user_addresses'
    AND column_name IN ('id', 'user_id', 'latitude', 'longitude', 'is_verified', 'is_primary');
    
    -- Verificar índices
    SELECT COUNT(*) INTO indexes_count
    FROM pg_indexes
    WHERE schemaname = 'public' 
    AND indexname LIKE 'idx_user_addresses_%';
    
    -- Verificar funciones
    SELECT COUNT(*) INTO functions_count
    FROM information_schema.routines
    WHERE routine_schema = 'public' 
    AND routine_name IN ('set_primary_address', 'find_nearby_addresses', 'update_address_usage');
    ...
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'Tablas: % de 2 (user_addresses, location_verification_log)', tables_count;
    RAISE NOTICE 'Columnas direcciones: % de 6 (columnas principales)', addresses_columns;
    RAISE NOTICE 'Índices: % (optimización de consultas)', indexes_count;
    RAISE NOTICE 'Funciones: % de 3 (gestión de direcciones)', functions_count;
    RAISE NOTICE 'Triggers: % (automatización)', triggers_count;
    RAISE NOTICE 'Políticas RLS: % (seguridad)', policies_count;
    RAISE NOTICE '==========================================';
    
    IF tables_count = 2 AND addresses_columns >= 6 AND functions_count = 3 THEN
        RAISE NOTICE 'SUCCESS: Sistema de ubicaciones instalado correctamente';
        RAISE NOTICE '✅ Gestión de múltiples direcciones por usuario';
        RAISE NOTICE '✅ Verificación GPS y Google Places';
        RAISE NOTICE '✅ Direcciones primarias automáticas';
        RAISE NOTICE '✅ Historial de verificaciones';
        RAISE NOTICE '✅ Instrucciones detalladas para repartidores';
        RAISE NOTICE '✅ Estadísticas de uso y éxito de entregas';
        RAISE NOTICE '✅ Funciones de búsqueda por proximidad';
        RAISE NOTICE '✅ Políticas de seguridad RLS';
        RAISE NOTICE '✅ Realtime para actualizaciones instantáneas';
        RAISE NOTICE '==========================================';
        ...
        RAISE NOTICE '3. Integración con Google Maps';
        RAISE NOTICE '4. Instrucciones para repartidores';
        RAISE NOTICE '5. Estadísticas de entrega';
        RAISE NOTICE '6. Búsqueda de direcciones cercanas';
        RAISE NOTICE '==========================================';
    ELSE
