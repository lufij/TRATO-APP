-- =====================================================
-- TRATO - FUNCIONES COMPLETAS PARA FLUJO DE ÓRDENES
-- =====================================================
-- Sistema completo de notificaciones y gestión de estados de órdenes

-- =====================================================
-- 1. FUNCIÓN PARA CAMBIAR ESTADO DE ORDEN
-- =====================================================
...
BEGIN
    ...
    -- Validar que el usuario tenga permisos para cambiar el estado
    IF p_new_status IN ('accepted', 'ready', 'rejected') AND p_user_id != v_order.seller_id THEN
        RETURN QUERY SELECT false, 'Solo el vendedor puede cambiar este estado'::TEXT, NULL::JSONB;
        RETURN;
    END IF;

    IF p_new_status IN ('assigned', 'picked-up', 'in-transit', 'delivered') AND p_user_id != v_order.driver_id AND NOT EXISTS (
        SELECT 1 FROM public.users WHERE id = p_user_id AND role = 'repartidor'
    ) THEN
...
-- =====================================================
-- 3. FUNCIÓN PARA OBTENER ÓRDENES DISPONIBLES PARA REPARTIDORES
-- =====================================================
...
BEGIN
    ...
    SELECT 
        ...
        o.estimated_time,
        o.created_at,
        NULL::DECIMAL(5,2) -- Placeholder para distancia (implementar con geolocalización)
    FROM public.orders o
...
SELECT 'Funciones del flujo de órdenes creadas exitosamente' as status;
