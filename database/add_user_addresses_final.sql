-- =================================================================
-- SISTEMA DE DIRECCIONES DE USUARIOS - VERSIÓN FINAL CORREGIDA
-- =================================================================
...
-- Totalmente compatible con PostgreSQL/Supabase
-- =================================================================
...
-- 3. AGREGAR COLUMNAS A TABLA USERS DE FORMA SEGURA
-- =================================================================
...
-- 4. AGREGAR COLUMNAS A TABLA ORDERS DE FORMA SEGURA
-- =================================================================
...
-- 11. CREAR CONSTRAINTS DE FORMA SEGURA
-- =================================================================

DO $$
BEGIN
    -- Constraint para una sola dirección primaria por usuario
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'idx_user_addresses_one_primary_per_user') THEN
        CREATE UNIQUE INDEX idx_user_addresses_one_primary_per_user
        ON user_addresses (user_id) WHERE (is_primary = TRUE);
    END IF;
    
    -- Foreign key para primary_address_id en users
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_users_primary_address') THEN
...
EXCEPTION WHEN OTHERS THEN
    -- Si hay algún error con los constraints, continuar
    RAISE NOTICE 'Advertencia: Algunos constraints no se pudieron crear: %', SQLERRM;
END $$;
...
-- 14. CONFIGURAR REALTIME (Opcional - puede fallar y es normal)
-- =================================================================

DO $$
BEGIN
    -- Intentar agregar tablas a realtime
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE user_addresses;
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Nota: No se pudo agregar user_addresses a realtime (normal si ya existe)';
    END;
    
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE location_verification_log;
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Nota: No se pudo agregar location_verification_log a realtime (normal si ya existe)';
    END;
END $$;

-- 15. MENSAJE DE CONFIRMACIÓN Y VERIFICACIÓN
-- =================================================================
...
BEGIN
    ...
    IF user_addresses_exists AND verification_log_exists AND functions_count >= 3 AND columns_count >= 3 THEN
        ...
        RAISE NOTICE 'Columnas en users: % de 4', columns_count;
        RAISE NOTICE '';
        RAISE NOTICE 'Funcionalidades disponibles:';
        RAISE NOTICE '• Múltiples direcciones por usuario con GPS';
        RAISE NOTICE '• Verificación automática de ubicaciones';
        ...
        RAISE NOTICE '• Gestión de direcciones primarias automática';
        RAISE NOTICE '';
        RAISE NOTICE 'Próximos pasos:';
        RAISE NOTICE '1. Recargar la aplicación web (Ctrl+F5)';
        RAISE NOTICE '2. Ir al perfil de comprador';
        RAISE NOTICE '3. Probar "Gestión de Ubicaciones"';
        RAISE NOTICE '4. Agregar direcciones con GPS';
        RAISE NOTICE '5. Probar checkout con selección de direcciones';
    ELSE
        RAISE NOTICE 'WARNING: ⚠️ Instalación parcial';
        RAISE NOTICE 'user_addresses: %', CASE WHEN user_addresses_exists THEN '✅' ELSE '❌' END;
        RAISE NOTICE 'location_verification_log: %', CASE WHEN verification_log_exists THEN '✅' ELSE '❌' END;
        RAISE NOTICE 'Funciones: % de 4', functions_count;
        RAISE NOTICE 'Columnas users: % de 4', columns_count;
        RAISE NOTICE '';
        RAISE NOTICE 'Verificar y re-ejecutar si es necesario';
    END IF;
    
    RAISE NOTICE '==========================================';
END $$;

-- 16. CONSULTA DE VERIFICACIÓN FINAL
-- =================================================================
...
SELECT 
    ...
    COUNT(*) as registros,
    'Direcciones de usuarios con GPS' as descripcion
FROM user_addresses
UNION ALL
SELECT 
    'location_verification_log' as tabla, 
    COUNT(*) as registros,
    'Historial de verificaciones GPS' as descripcion
FROM location_verification_log
UNION ALL
SELECT 
    'Columnas agregadas a users' as tabla,
    COUNT(*) as registros,
    'current_latitude, current_longitude, etc.' as descripcion
FROM information_schema.columns
WHERE table_schema = 'public' 
AND table_name = 'users'
AND column_name IN ('current_latitude', 'current_longitude', 'location_permission_granted', 'primary_address_id')
UNION ALL
SELECT 
    'Funciones GPS creadas' as tabla,
    COUNT(*) as registros,
    'calculate_distance, set_primary_address, etc.' as descripcion
FROM information_schema.routines
WHERE routine_schema = 'public' 
AND routine_name IN ('calculate_distance_accurate', 'set_primary_address', 'find_nearby_addresses', 'update_address_usage');