...
IF NOT EXISTS (
    ...
    RAISE NOTICE 'Foreign key user_id agregado (este SÍ es necesario)';
END IF;
...
RAISE NOTICE '%', '✓ Funciones de validación creadas';
...
BEGIN
    ...
    RAISE NOTICE '%', format('   • Orders foreign keys: %s/3 (buyer, seller, driver)', orders_fk_count);
    RAISE NOTICE '%', format('   • Cart foreign keys problemáticos: %s (debe ser 0)', cart_problematic_fk_count);
    RAISE NOTICE '%', format('   • Cart foreign keys seguros: %s (user_id)', cart_safe_fk_count);
    RAISE NOTICE '%', format('   • Funciones de validación: %s/3', cart_functions_count);
    RAISE NOTICE '%', format('   • Columnas nuevas en cart: %s/5', cart_columns_count);
    RAISE NOTICE '%', format('   • Tablas del sistema orders: %s/4', tables_count);
    RAISE NOTICE '%', '';
    RAISE NOTICE '%', 'ERRORES SOLUCIONADOS:';
    RAISE NOTICE '%', '   ✓ "Could not find relationship orders/users" → CORREGIDO';
    RAISE NOTICE '%', '   ✓ "cart_items violates foreign key constraint" → ELIMINADO';
