-- =====================================================
-- TRATO - SOLUCIÓN AL ERROR DE FOREIGN KEY EN CARRITO (VERSIÓN CORREGIDA)
-- =====================================================
-- Este script soluciona el error de foreign key constraint en cart_items
-- VERSIÓN CORREGIDA: Arregla error de sintaxis con palabra reservada "exists"
...
-- =====================================================
-- 3. FUNCIÓN PARA VALIDAR Y OBTENER DATOS DEL PRODUCTO (CORREGIDA)
-- =====================================================
...
RETURNS TABLE (
    ...
    image_url TEXT,
    seller_id UUID,
    product_exists BOOLEAN  -- Cambiado de "exists" a "product_exists"
) AS $$
DECLARE
    v_count INTEGER := 0;
BEGIN
    IF p_product_type = 'daily' THEN
        -- Buscar en daily_products y retornar directamente
        RETURN QUERY
        SELECT 
            dp.id,
            dp.name,
    ...
    ELSE
        -- Buscar en products regulares y retornar directamente
        RETURN QUERY
        SELECT 
            p.id,
            p.name,
...
-- =====================================================
-- 4. FUNCIÓN PARA AGREGAR AL CARRITO CON VALIDACIÓN (CORREGIDA)
-- =====================================================
...
BEGIN
    ...
    -- Obtener detalles del producto usando la función corregida
    SELECT * INTO product_details FROM get_product_details(p_product_id, p_product_type);
    
    IF NOT product_details.product_exists THEN  -- Cambiado de "exists" a "product_exists"
...
BEGIN
    ...
    RAISE NOTICE '%', '   ✓ Error de sintaxis "exists" corregido';
