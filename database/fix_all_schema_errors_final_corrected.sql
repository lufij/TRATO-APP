-- ============================================================================
-- TRATO - Solución Definitiva para Errores de Esquema (CORREGIDA)
-- ============================================================================
...
-- 4. ERROR: column "last_message_id" referenced in foreign key constraint does not exist
-- ============================================================================

DO $$
BEGIN
    RAISE NOTICE '🔧 INICIANDO CORRECCIÓN DE ERRORES DE ESQUEMA (VERSIÓN CORREGIDA)...';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '  4. ERROR: column "last_message_id" referenced in foreign key constraint does not exist';
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'notifications') THEN
        RAISE NOTICE '➕ Creando tabla notifications completa...';
    ...
    ELSE
        RAISE NOTICE '🔍 Verificando estructura de notifications existente...';
        ...
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'user_id') THEN
            -- Si no existe recipient_id, renombrar user_id a recipient_id
            ...
            ELSE
                RAISE NOTICE '⚠️  Ambas columnas user_id y recipient_id existen. Eliminando user_id...';
                ALTER TABLE notifications DROP COLUMN user_id;
                RAISE NOTICE '✅ Columna user_id eliminada';
            END IF;
        END IF;
        
        -- Verificar si existe recipient_id, si no, crearla
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'recipient_id') THEN
        ...
        -- Verificar otras columnas necesarias y agregarlas si faltan
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'created_at') THEN
    ...
    -- Crear índices para notifications
...
BEGIN
    ...
    -- Crear tabla conversations si no existe (SIN last_message_id por ahora)
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'conversations') THEN
        ...
        RAISE NOTICE '✅ Tabla conversations creada (sin last_message_id aún)';
    ELSE
        RAISE NOTICE '✅ Tabla conversations ya existe';
        
        -- Asegurar que tiene las columnas básicas
    ...
    -- AHORA agregar last_message_id a conversations (después de que messages existe)
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'conversations' AND column_name = 'last_message_id') THEN
        ...
        RAISE NOTICE '✅ Columna last_message_id agregada';
    END IF;
    
    -- AHORA crear la foreign key constraint (después de que la columna existe)
    IF NOT EXISTS (
    ...
    ELSE
        RAISE NOTICE '✅ Foreign key conversations → messages ya existe';
    END IF;
...
BEGIN
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'seller_id') THEN
            ALTER TABLE orders ADD COLUMN seller_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE;
        END IF;
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'status') THEN
            ALTER TABLE orders ADD COLUMN status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'confirmed', 'preparing', 'ready', 'delivered', 'cancelled'));
        END IF;
    ...
    ELSE
        ...
        IF NOT EXISTS (
            ...
            -- Primero verificar que la tabla products existe
            ...
            ELSE
                ...
                RAISE NOTICE '✅ Tabla products creada y foreign key agregada';
            END IF;
        ELSE
...
DECLARE
    ...
    fk_conversations_messages BOOLEAN := FALSE;
...
BEGIN
    ...
    ) INTO fk_conversations_messages;
    ...
    -- ERROR 2: Chat tables y last_message_id
    RAISE NOTICE '2️⃣ ERROR CHAT TABLES:';
    IF conversations_exists THEN
        RAISE NOTICE '   ✅ Tabla conversations: EXISTE';
        IF last_message_id_exists THEN
            RAISE NOTICE '   ✅ Columna last_message_id: EXISTE';
            IF fk_conversations_messages THEN
                RAISE NOTICE '   ✅ FK conversations → messages: EXISTE';
            ELSE
                RAISE NOTICE '   ❌ FK conversations → messages: FALTA';
                error_count := error_count + 1;
            END IF;
        ELSE
            RAISE NOTICE '   ❌ Columna last_message_id: FALTA';
            error_count := error_count + 1;
        END IF;
    ELSE
    ...
    IF error_count = 0 THEN
        ...
        RAISE NOTICE '✅ Error last_message_id constraint: SOLUCIONADO';
    ...
    ELSE
        ...
        RAISE NOTICE '📞 Revisa los logs detallados arriba para más información.';
    END IF;
